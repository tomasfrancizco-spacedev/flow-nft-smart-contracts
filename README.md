...
4. Run `mint_nft` signing with the NFT owner's account and the receiver's account as parameter

  ```
  $ flow transactions send cadence/transactions/mint_nft.cdc 83505dadaf7e2161 1 1 1 \
    --signer testnet-account \
    --network testnet
  ```

5. You can check collections, series, sets and nfts with the `check scripts`: `$ flow scripts execute cadence/scripts/check_series.cdc 1`
